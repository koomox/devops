#!/bin/bash
cd /tmp
wget -O nginx-1.22.0.tar.gz https://raw.githubusercontent.com/koomox/devops/master/storage/linux/scripts/nginx/1.22.0/binary/nginx-1.22.0.tar.gz
tar -zxf nginx-1.22.0.tar.gz
\rm -rf /usr/local/nginx /etc/nginx
mv nginx-1.22.0 /usr/local/nginx
cd /usr/local/nginx
mv conf /etc/nginx
