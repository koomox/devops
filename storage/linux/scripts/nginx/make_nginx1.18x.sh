#!/bin/bash
NGINX_VERSION=$(wget -qO- --no-check-certificate https://nginx.org/en/download.html | grep -m1 -E "nginx-1.18.([0-9]+).tar.gz" | sed -E "s/.*(nginx-1.18.[0-9]+).tar.gz.*/\1/gm" )
OPENSSL_VERSION=$(wget -qO- --no-check-certificate https://www.openssl.org/source/ | grep -m1 -E "\"openssl-1(\.[0-9]+){0,2}\w+.tar.gz\"" | sed -E "s/.*\"(openssl-.*).tar.gz\".*/\1/gm" )
...
wget https://ftp.exim.org/pub/pcre/${PCRE_VERSION}.tar.gz
