...
echo -e "vm.swappiness = 0" | ${SUDO} tee /etc/sysctl.conf
echo -e "net.ipv4.neigh.default.gc_stale_time=120" | ${SUDO} tee -a /etc/sysctl.conf
echo -e "net.ipv4.conf.all.rp_filter=0" | ${SUDO} tee -a /etc/sysctl.conf
...
echo "================= Public IP ====================="
echo $(curl https://ddns.oray.com/checkip)